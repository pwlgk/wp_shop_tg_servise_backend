...
async def test_simple_expiration(db_session, test_user, mocker):
    ...
    # Создаем одну просроченную транзакцию
...
async def test_no_expiration_if_spent(db_session, test_user, mocker):
    ...
    crud_loyalty.create_transaction(db_session, user_id=test_user.id, points=100, type='order_earn', created_at=datetime.now(timezone.utc) - timedelta(days=10), expires_at=datetime.now(timezone.utc) - timedelta(days=1))
    crud_loyalty.create_transaction(db_session, user_id=test_user.id, points=50, type='admin_adjust', created_at=datetime.now(timezone.utc) - timedelta(days=8))
    crud_loyalty.create_transaction(db_session, user_id=test_user.id, points=-120, type='order_spend', created_at=datetime.now(timezone.utc) - timedelta(days=5))
    db_session.commit()
